...
int RotateModel(Table *table, const Rotate &act)
{
    t_matrix m;
	...
	while (move && !ret)
	{
		ret = Change_Point_with_matrix(*move->cut->getBegin(), m);
		ret = Change_Point_with_matrix(*move->cut->getEnd(), m);
		move = move->next;
	}
   return ret;
}

int ScaleModel(Table *table, const Scale &act)
{
    t_matrix m;
    GetResultMatrix(m, act);
    int ret = 0;
